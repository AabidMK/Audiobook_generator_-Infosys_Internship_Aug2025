...
# Config
# ----------------------------
# Use data/chroma_db if it exists, otherwise fall back to chroma_db
import os
PERSIST_DIR = "data/chroma_db" if os.path.exists("data/chroma_db") else "chroma_db"
...
try:
    ...
    print(f"Collection '{COLLECTION_NAME}' not found. Please run indexing first.")
    collection = None
...
def rag_pipeline(user_question: str, top_k: int = 5):
    if collection is None:
        return "No documents indexed. Please run indexing first.", []

...
class EnhancedTextExtraction:
        ...
        try:
            ...
            for page_num in range(total_pages):
                ...
                if page_num % 10 == 0 and page_num > 0:
                    print(f"[CLEANUP] Memory cleanup at page {page_num}")
                    import gc
                    gc.collect()
                
                # Safety break for very large documents
                if page_num > 100:
                    print("[LIMIT] Limiting to first 100 pages for safety")
                    break
            
            doc.close()
            
            final_text = '\n\n'.join(full_text)
            print(f"[SUCCESS] Extraction complete: {len(final_text):,} characters")
            
            # If no text found, try OCR on all pages
            if not final_text or len(final_text.strip()) < 50:
                print("[OCR] No text found, trying OCR on all pages...")
                ocr_text = []
                
                for page_num in range(min(total_pages, 10)):  # Limit OCR to first 10 pages
                    try:
                        page = doc[page_num]
                        pix = page.get_pixmap(matrix=fitz.Matrix(2, 2))  # 2x zoom for better OCR
                        img_data = pix.tobytes("png")
                        
                        ocr_result = EnhancedTextExtraction.extract_text_from_image(img_data, is_bytes=True)
                        if ocr_result and not ocr_result.startswith("Error") and len(ocr_result.strip()) > 20:
                            ocr_text.append(f"--- Page {page_num + 1} OCR ---")
                            ocr_text.append(ocr_result)
                            print(f"[OCR] Page {page_num + 1}: {len(ocr_result)} chars")
                ...
                if ocr_text:
                    final_text = '\n\n'.join(ocr_text)
                    print(f"[OCR SUCCESS] Total OCR text: {len(final_text):,} characters")
            
            return final_text if final_text and len(final_text.strip()) > 10 else "No readable text found in PDF"
    ...
    def extract_text_from_pdf_basic(file_path: str) -> str:
        ...
        try:
            ...
            with open(file_path, 'rb') as file:
                ...
                print(f"[FALLBACK] Fallback extraction: {total_pages} pages")
                ...
                for i, page in enumerate(reader.pages):
                    try:
                        ...
                        # Safety limit
                        if i > 50:
                            print("[LIMIT] Limiting to first 50 pages")
                            break
                ...
                if not final_text or len(final_text.strip()) < 50:
                    print("[OCR FALLBACK] Trying OCR extraction...")
                    try:
                        import fitz
                        doc = fitz.open(file_path)
                        ocr_results = []
                        
                        for page_num in range(min(len(reader.pages), 5)):  # OCR first 5 pages
                            try:
                                page = doc[page_num]
                                ...
                                if ocr_text and len(ocr_text.strip()) > 20:
                                    ...
                                    print(f"[OCR] Page {page_num + 1}: extracted")
                        ...
                        if ocr_results:
                            final_text = '\n\n'.join(ocr_results)
                            print(f"[OCR SUCCESS] Extracted {len(final_text)} characters via OCR")
                    
                    except Exception as e:
                        print(f"[OCR FAILED] {e}")
                
                return final_text if final_text and len(final_text.strip()) > 10 else "No readable content found"
    ...
    def extract_text_from_any(file_path: str) -> str:
        ...
        print(f"[PROCESSING] Processing: {os.path.basename(file_path)}")
        ...
        try:
            if file_extension == '.pdf':
                ...
                print("[FALLBACK] Trying fallback PDF extraction...")
                fallback_result = EnhancedTextExtraction.extract_text_from_pdf_basic(file_path)
                
                # If still no content, return a helpful message
                if not fallback_result or fallback_result.startswith("Error") or len(fallback_result.strip()) < 50:
                    return "This PDF appears to be image-based or contains no extractable text. The document may need OCR processing or manual conversion."

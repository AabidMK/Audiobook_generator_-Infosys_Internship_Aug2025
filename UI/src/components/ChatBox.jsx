...
export default function ChatBox({ initialAnswer }) {
  ...
  return (
    <div style={{ borderRadius: 12, background: "#535353ff", padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
      <div style={{ minHeight: 200, maxHeight: 360, overflowY: "auto", display: "flex", flexDirection: "column", gap: 8 }}>
        {messages.map((m, i) => (
          <div key={i} style={{
            background: m.role === "user" ? "#c8e6c9" : (m.role==="assistant" ? "#bbdefb" : "#fff"),
            padding: 10,
            borderRadius: 8
          }}>
            <strong style={{ display:"block", fontSize:12, color:"#4e4e4eff" }}>{m.role}</strong>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        />
        <button onClick={ask} disabled={loading} style={{ padding: "6px 12px", borderRadius: 6, background: "#0288d1", color: "#fff", border: "none" }}>
          {loading ? "..." : "Ask"}
        </button>
      </div>
    </div>

...
# ✅ Ensure local src folder is visible to Python
CURRENT_DIR = Path(__file__).resolve().parent
SRC_DIR = CURRENT_DIR / "src"
if str(SRC_DIR) not in sys.path:
    sys.path.append(str(SRC_DIR))

# ✅ Fix for relative imports (works when called from FastAPI too)
os.environ["CHROMA_DEFAULT_EMBEDDING_FUNCTION"] = "sentence-transformers/all-MiniLM-L6-v2"

# ✅ Now these imports will work from both CLI & FastAPI
from document_parser import parse_dir, parse_file
from text_chunker import TextChunker
from embedding_manager import EmbeddingManager
from vector_store import VectorStoreManager
from ollama_llm import OllamaLLMAnswerGenerator
...
def index_documents(input_path, backend="chroma", model="sentence-transformers/all-MiniLM-L6-v2"):
    ...
    ids=[str(uuid.uuid4()) for _ in range(len(texts))],  # ✅ use unique IDs
...
def query_docs(question: str,
    ...
    "If the context does not include the answer, say 'I could not find that information in the document.'\n"
    "Write answers clearly and accurately, without guessing or adding information.\n\n"
    f"### CONTEXT:\n{context}\n\n"
    f"### QUESTION:\n{question}\n\n"
    "### ANSWER:"
)
